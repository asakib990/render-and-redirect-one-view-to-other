...
class SeApplication(models.Model):
    ...
    application_photo = fields.Image(string='Photo', attachment=True, store=True)
    ...
    #                              string='Student ID')
    # register_id = fields.Many2one(comodel_name='se.application.register',
    #                               string='Register ID')
    ...
    # semester_type_id = fields.Char('Semester Type')
    ...
    eligibility_application_state = fields.Selection([
        ('uncheck', 'Uncheck'),
        ('agree', 'Agree'),
        ('disagree', 'Disagree')
    ...
    is_admission_applicant_admitted = fields.Boolean(string="Is Admission Applicant Admitted?",
                                                     default=False,
                                                     readonly=True,
    ...
    prev_transcript = fields.Binary(string='Previous Transcript', attachment=True, store=True,
                                    help="application Previous Transcript.")
    ...
    ssc_certificate = fields.Binary(string='SSC Academic Transcript', attachment=True, store=True,
                                    help="application SSC Certificate.")
    ...
    # education_board_ssc_id = fields.Many2one(comodel_name='se.education.board',
    #                                          string='Education Board')
    # admission_board_result_ssc_ids = fields.One2many(comodel_name='se.application.board.result.general',
    ...
    hsc_certificate = fields.Binary(string='HSC Academic Transcript', attachment=True, store=True,
                                    help="application HSC Certificate.")
    ...
    # admission_board_result_hsc_ids = fields.One2many(comodel_name='se.application.board.result.general',
    #                                                  inverse_name='application_id',
    #                                                  string='Admission Board Results of HSC',
    ...
    o_level_certificate = fields.Binary(string='O-Level Academic Transcript', attachment=True, store=True,
                                        help="application O-Level Certificate.")
    # admission_board_result_o_level_ids = fields.One2many(comodel_name='se.application.board.result.english',
    #                                                      inverse_name='application_id',
    ...
    a_level_certificate = fields.Binary(string='A-Level Academic Transcript', attachment=True, store=True,
                                        help="application A-Level Certificate.")
    # admission_board_result_a_level_ids = fields.One2many(comodel_name='se.application.board.result.english',
    #                                                      inverse_name='application_id',
    ...
    # admission_board_result_all_ids = fields.One2many(comodel_name='se.application.board.result.all',
    #                                                  inverse_name='application_id',
    #                                                  string='Admission Board Results')

    # Achievement & Experience
    # admission_achievement_ids = fields.One2many(comodel_name='se.application.achievement',
    #                                             inverse_name='application_id',
    #                                             string='Admission application Achievements')
    # admission_professional_experience_ids = fields.One2many(comodel_name='se.application.professional.experience',
    #                                                         inverse_name='application_id',
    #                                                         string='Admission application Professional Experiences')

    # Personal Details

    signature = fields.Binary(string='Signature', attachment=True, store=True, help="application signature.")
    passport = fields.Binary(string='Passport', attachment=True, store=True, help="application passport.")
    ...
    # admission_fee_account_move_id = fields.Many2one(comodel_name="account.move",
    #                                                 string="Invoice")
    ...
    # def update_user_password_admission_application_student(self):
    #     for record in self:
    ...
    def enroll_student(self):
        for record in self:
            ...
            # super(Seapplication, self).enroll_student()

            # ## Create invoice for Admission Fee
            # self.create_invoice_admission_application_admission_fee()

            ## Update Student login Password using Mobile number
            self.update_user_password_admission_application_student()

            ## Update Partner
            # if record.partner_id:
            #     record.partner_id.sudo().write({
            #         'email': self.email,
            #         'is_admission_application': True,
            #         'is_student': True,
            #     })
            #
    ...
    #     application = self.env['se.application'].browse(self.id)
    #     if application:
    ...
    #         self.env["se.application.board.result.general"].search(
    #             [('exam_name_ref', '=', 'ssc'), ('application_id', '=', self.id)]).unlink()
    #         if result:
    #             for item in result['subject']:
    #                 self.env["se.application.board.result.general"].sudo().create({
    ...
    #         self.env["se.application.board.result.general"].search(
    #             [('exam_name_ref', '=', 'hsc'), ('application_id', '=', self.id)]).unlink()
    #         if result:
    #             for item in result['subject']:
    #                 self.env["se.application.board.result.general"].sudo().create({
    #                     'subject_name': str(item['subName']),
    ...
    # Create Invoice
    # def create_invoice_admission_application_admission_fee(self):
    #     for record in self:
    ...
    #                 'ref': str(record.id),
    #                 'is_admission_application_invoice': True,
    ...
    # def action_admission_applicant_admitted_wizard(self):
    #     if not self.student_id:
    #         raise UserError("Student profile is not created, yet.")
    #
    #     if not self.student_id_text:
    #         raise UserError("Student ID is not assign.")
    #
    #     return {
    #         'name': _('Propose Student Email'),
    #         'type': 'ir.actions.act_window',
    #         'view_type': 'form',
    #         'view_mode': 'form',
    #         'view_id': self.env.ref('se_admission.student_email_proposed_wizard_form_view').id,
    #         'res_model': 'object',
    #         'res_model': 'student.email.proposed.wizard',
    #         'target': 'new',
    #         'context': {
    #             'admission_id': self.id,
    #             'default_student_name': str(self.student_id.name),
    #             'default_student_id': str(self.student_id.student_id),
    #         },
    #     }